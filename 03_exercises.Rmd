...
> The problem is that the same vegetable with the same variety was planted in different plot. {?}  
...
Trips %>%
  mutate(sd_week = wday(sdate,label = TRUE),
         ...
         sd_minute = minute(sdate),
         sd_time = sd_hour + sd_minute/60) %>% 
  ggplot(aes(y = sd_time,fill = client)) + 
...
Trips %>% 
  ...
  ggplot(aes(x = per_casual, y = sstation)) +
  geom_col()
```
